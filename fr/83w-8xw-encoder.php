<?
  ...
  $filetype = isset($_GET["type"]) ? $_GET["type"] : "8xw";
    if ($filetype == "8xw")
      {
        //83P/84 Family
        if ($onti_calctype == '83P')
          {
              ...
              $entete[53] = ($listcount * 9 + 20) & 0xFF;
              $entete[54] = (($listcount * 9 + 20) >> 8) & 0xFF;
              $entete[55] = 0xD;
              ...
              $final_filename = "WindowRange.tizw.8xw";
        ...
        //83 Family
        else if ($onti_calctype == '83')
          {
              ...
              $final_filename = "WindowRange.tizw.83w";
              header( 'Content-Type: application/octet-stream' );
              header('Content-Disposition: attachment; filename="'.$final_filename.'"');
              header("Last-Modified: " . gmdate("D, d M Y H:i:s") . " GMT");
              readfile(session_id().'.dat');   
