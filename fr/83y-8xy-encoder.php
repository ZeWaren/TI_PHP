<?
  ...
  $filetype = isset($_GET["type"]) ? $_GET["type"] : "8xy";
    if ($filetype == "8xy")
      {
        //83P/84 Family
        if ($onti_calctype == '83P')
          {
              ...
              $entete[53] = (count($content)+2+0x11) & 0xFF;
              $entete[54] = ((count($content)+2+0x11) >> 8) & 0xFF;
              $entete[55] = 0xD;
              $entete[57] = 0x12;
              $entete[59] = 0x3;
              $entete[60] = 0x5E;
...
/*
            if ($contents < 0x1A)
              ...
              $TIName = "v";
*/
              $names = array ('Y1', 'Y2', 'Y3', 'Y4', 'Y5', 'Y6', 'Y7', 'Y8', 'Y9', 'Y0',
                             'X1T', 'Y1T', 'X2T', 'Y2T', 'X3T', 'Y3T', 'X4T', 'Y4T', 'X5T', 'Y5T', 'X6T', 'XYT',
                             'r1', 'r2', 'r3', 'r4', 'r5', 'r6',
                             'u', 'v');
              $value = array (0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 
                              0x20, 0x21, 0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2A, 0x2B,
                              0x40, 0x41, 0x42, 0x43, 0x44, 0x45,
                              0x80, 0x81);

              for ($i=0; $i<count($names); $i++)
                if ($names[$i] == $onti_filename)
                  {
                    $tinumber = $value[$i];
                    break;
                  }
              ...
              $final_filename = "$onti_filename".".tizw.8xy";
        ...
        //83 Family
        else if ($onti_calctype == '83')
          {
              ...
              $final_filename = "$onti_filename".".tizw.83y";
              header( 'Content-Type: application/octet-stream' );
              header('Content-Disposition: attachment; filename="'.$final_filename.'"');
              header("Last-Modified: " . gmdate("D, d M Y H:i:s") . " GMT");
              readfile(session_id().'.dat');
