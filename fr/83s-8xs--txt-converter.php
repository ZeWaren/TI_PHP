...
<?
 ...
 $ffilename=session_id();
   if ( (!isset($_FILES['userfile'])) || (!move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadDir . $ffilename)))
     {
       ...
       echo '<form enctype="multipart/form-data" action="83s-8xs--txt-converter.php" method="post">';
       echo '<input type="hidden" name="MAX_FILE_SIZE" value="5000000" />';
       echo 'Ouvrir une chaîne TI (83s, 8xs): <input name="userfile" type="file" /> <input type="submit" value="Ouvrir">';
       echo '</form>';  
       include "bottom.php";
       die();
     }
  ...
  if ($contents == "**TI83F*")
    {
          ...
          if (ord($contents) != 0xAA)
            {
               echo 'File Problem - Doesn\'t seem to be a String File<br>';
               echo '<a href="index.php">Retour au menu principal</a>';
               echo '<h2>Ouvrir un fichier</h2>';
               echo '<form enctype="multipart/form-data" action="83s-8xs--txt-converter.php" method="post">';
               echo '<input type="hidden" name="MAX_FILE_SIZE" value="5000000" />';
          ...
          fseek($handle,61);
          $contents = fread($handle, 1);
          $TIName = 'Str'.(substr(ord($contents).ord($contents)+1, 1,1));
  ...
  echo '<div style="float: left;"><a href="83s-8xs-file-as-text-file.php"><img src="./images/extensions/txt.png"> Enregistrer en TXT</a></div>';
  echo '<div style="float: left;">&nbsp;&nbsp;&nbsp;&nbsp;<a href="83s-8xs-file-as-text-file.php?noheader=true"><img src="./images/extensions/txt.png"> Enregistrer en TXT sans les informations d\'entête</a></div>';
