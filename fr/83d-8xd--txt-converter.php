...
<?
 ...
 $ffilename=session_id();
   if ( (!isset($_FILES['userfile'])) || (!move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadDir . $ffilename)))
     {
       ...
       echo '<form enctype="multipart/form-data" action="83d-8xd--txt-converter.php" method="post">';
       echo '<input type="hidden" name="MAX_FILE_SIZE" value="5000000" />';
       echo 'Ouvrir un GDB (83d, 8xd): <input name="userfile" type="file" /> <input type="submit" value="Ouvrir">';
       echo '</form>';
       include "bottom.php";
       die();
     }
  ...
  if ($contents == "**TI83F*")
    {
         ...
         function ReadFunction()
         {
          ...
          fseek($handle,61);
          $contents = fread($handle, 1);
          $TIName = 'GDB'.(substr(ord($contents).ord($contents)+1, 1,1));          
            ...
            if ($graphingmode == 0x10)
              {
                ...
                for ($i=0; $i<10; $i++)
                  {
                    $graphstyles[$i] = ord(fread($handle, 1));
                  }
                ...
                $text2 = $text."\r\n\r\n";
                $text .= "\r\n\r\n<table><tr><td><u>Nom</u></td><td><u>Style</u></td><td><u>Etat</u></td><td><u>Contenu</u></td></tr>";
                for ($i=0; $i<10; $i++)
                  {
                    $text .= "<tr>";
                    $text .= "<td>Y".($i == 9 ? "0" : $i + 1)."</td>";
                    $text .= "<td>".$graphstyle[$graphstyles[$i]]."</td>";
                    $text .= "<td>".($selecteds[$i] == 0x23 ? "selected " : "unselected")."</td>";
                    $text .= "<td>".$functions[$i]."</td>";
                    $text .= "</tr>";
                    $text2 .= "Y".($i == 9 ? "0" : $i + 1)."\t".$graphstyle[$graphstyles[$i]]."\t".($selecteds[$i] == 0x23 ? "selected " : "unselected")."\t".$functions[$i]."\r\n";
                  }
                $text .= "</table>";
              }
            else if ($graphingmode == 0x40)
              {
                ...
                $text2 = $text."\r\n\r\n";
                $text .= "\r\n\r\n<table><tr><td><u>Index</u></td><td><u>Style</u></td><td><u>Etat</u></td><td><u>XxT</u></td><td><u>YxT</u></td></tr>";
                $count = 0;
                for ($i=0; $i<6; $i++)
                  {
                    $text .= "<tr>";
                    ...
                    $text .= "<td>".$functions[$count++]."</td>";
                    $text .= "</tr>";
                    $text2 .= ($i == 9 ? "0" : $i + 1)."\t".$graphstyle[$graphstyles[$i]]."\t".($selecteds[$i] == 0x23 ? "selected " : "unselected")."\t".$functions[$count-2]."\t".$functions[$count-1]."\r\n";
                  }
                $text .= "</table>";
              }
                ...
                for ($i=0; $i<6; $i++)
                  {
                    ...
                    $text .= "<td>".$functions[$i]."</td>";
                    $text .= "</tr>";
                    $text2 .= "r".($i == 9 ? "0" : $i + 1)."\t".$graphstyle[$graphstyles[$i]]."\t".($selecteds[$i] == 0x23 ? "selected " : "unselected")."\t".$functions[$i]."\r\n";
                  }
                $text .= "</table>";
              }
            else if ($graphingmode == 0x80)
              {
                ...
                $text .= "\r\nu(vMin), first element: ".ReadNumber();
                $text .= "\r\nnMin: ".ReadNumber();
                $text .= "\r\nu(nMin), second element: ".ReadNumber();
                $text .= "\r\nu(vMin), second element: ".ReadNumber();
                $text .= "\r\nw(nMin), first element: ".ReadNumber();
                $text .= "\r\nPlot Step: ".ReadNumber();
                ...
                for ($i=0; $i<3; $i++)
                  {
                    ...
                    $text .= "<td>".$functions[$i]."</td>";
                    $text .= "</tr>";
                    $text2 .= $functionnames[$i]."\t".$graphstyle[$graphstyles[$i]]."\t".($selecteds[$i] == 0x23 ? "selected " : "unselected")."\t".$functions[$i]."\r\n";
                  }
                $text .= "</table>";
              }
    }
  ...
  echo '<li>Nom de la variable:  <font color="DimGray">"'.$TIName.'"</font></li>';
  ...
  echo '<div style="float: left;"><a href="83d-8xd-file-as-text-file.php"><img src="./images/extensions/txt.png"> Enregistrer en TXT</a></div>';
  echo '<div style="float: left;">&nbsp;&nbsp;&nbsp;&nbsp;<a href="83d-8xd-file-as-text-file.php?noheader=true"><img src="./images/extensions/txt.png"> Enregistrer en TXT sans les informations d\'entête</a></div>';
